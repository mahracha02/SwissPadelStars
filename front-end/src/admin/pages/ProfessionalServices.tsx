...
export default function ProfessionalServices() {
  ...
  const [currentService, setCurrentService] = useState<ProfessionalService | null>(null);
  ...
  const columns = [
    { 
      key: 'title' as keyof ProfessionalService, 
      label: 'Titre',
      render: (value: string) => (
    ...
    { 
      key: 'description' as keyof ProfessionalService, 
      label: 'Description',
      render: (value: string) => (
        <div className="max-w-md">
          <p className="truncate" title={value}>
    ...
    { 
      key: 'image' as keyof ProfessionalService, 
      label: 'Image',
      render: (value: string) => (
        value ? (
          <img
    ...
    {
      key: 'published' as keyof ProfessionalService,
      label: 'Publié',
  ...
  const fetchServices = async () => {
    try {
      const response = await fetch('https://127.0.0.1:8000/api/admin/professional-services');
      if (!response.ok) {
  ...
  const handleDelete = (service: ProfessionalService) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`https://127.0.0.1:8000/api/admin/professional-services/${serviceToDelete.id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleTogglePublish = async (service: ProfessionalService) => {
    try {
      const response = await fetch(`https://127.0.0.1:8000/api/admin/professional-services/${service.id}/toggle-publish`, {
        method: 'PATCH',
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (currentService) {
        const response = await fetch(`https://127.0.0.1:8000/api/admin/professional-services/${currentService.id}`, {
          method: 'PUT',
          headers: {
