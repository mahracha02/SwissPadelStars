...
export default function ProfessionalServices() {
  ...
  const fetchServices = async () => {
    try {
      const response = await axios.get('/api/admin/professional-services');
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this service?')) {
      try {
        await axios.delete(`/api/admin/professional-services/${id}`);
        fetchServices();
      } catch (error) {
  ...
  const handleTogglePublish = async (id: number, currentStatus: boolean) => {
    try {
      await axios.patch(`/api/admin/professional-services/${id}`, {
        published: !currentStatus,
      });
      fetchServices();
    } catch (error) {
      console.error('Error toggling professional service status:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedService) {
        await axios.put(`/api/admin/professional-services/${selectedService.id}`, formDataToSend);
      } else {
        await axios.post('/api/admin/professional-services', formDataToSend);
      }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Professional Services</h1>
        <button
